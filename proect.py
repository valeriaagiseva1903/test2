...
def init_main(self):
    ...
    self.add_image = tk.PhotoImage(file='./image/delete.png')

def init_main(self):
    toolbar = tk.Frame(bg='#d7d8e0', bd=2)
    toolbar.pack(side=tk.TOP, fill=tk.X)
    
    self.add_image = tk.PhotoImage(file='./image/refresh.png')

def init_main(self):
    toolbar = tk.Frame(bg='#d7d8e0', bd=2)
    toolbar.pack(side=tk.TOP, fill=tk.X)
    
    self.add_image = tk.PhotoImage(file='./image/search.png')

def init_main(self):
    toolbar = tk.Frame(bg='#d7d8e0', bd=2)
    toolbar.pack(side=tk.TOP, fill=tk.X)
    
    self.add_image = tk.PhotoImage(file='./image/update.png')
...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        btn_open_dialog = tk.Button(toolbar, bg='#d7d8e0', bd=0, image=self.add_image, command=self.open_dialog)
        ...
        self.update_image = tk.PhotoImage(file='./image/red.png')
        btn_edit_dialog = tk.Button(toolbar, bg='#d7d8e0', bd=0, image=self.update_image, command=self.open_update_dialog)
        btn_edit_dialog.pack(side=tk.LEFT)

        #создание кнопки удаления записи
        self.delete_image = tk.PhotoImage(file='./image/del.png')
        btn_delete = tk.Button(toolbar, bg='#d7d8e0', bd=0, image=self.delete_image, command=self.delete_record)
        btn_delete.pack(side=tk.LEFT)

        #создание кнопки поиска записи
        self.search_image = tk.PhotoImage(file='./image/fnd.png')
        btn_search = tk.Button(toolbar, bg='#d7d8e0', bd=0, image=self.search_image, command=self.open_search_dialog)
        btn_search.pack(side=tk.LEFT)

        #создание кнопки обновления записи
        self.refresh_image = tk.PhotoImage(file='./image/upd.png')
        btn_refresh = tk.Button(toolbar, bg='#d7d8e0', bd=0, image=self.refresh_image, command=self.view_records)
